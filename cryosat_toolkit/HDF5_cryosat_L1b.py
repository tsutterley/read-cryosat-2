...
def HDF5_cryosat_L1b(CS_l1b_mds, MODE, BASELINE, FILENAME='', TITLE='',
	...
	for key in Location_keys:
		...
		for att_name,att_val in CS_l1b_attrib['Location'][key].iteritems():
	...
	for key in Data_keys:
		...
		#-- add HDF5 variable attributes
		for att_name,att_val in CS_l1b_attrib['Data'][key].iteritems():
	...
	for key in Geometry_keys:
		...
		#-- add HDF5 variable attributes
		for att_name,att_val in CS_l1b_attrib['Geometry'][key].iteritems():
	...
	for key in Wfm_1Hz_keys:
		...
		if key in ('Waveform'):
			...
			h5['Waveform_1Hz'][key].dims[0].label='CS_L1b_MDS_REC_SIZE'
		#-- add HDF5 variable attributes
		for att_name,att_val in CS_l1b_attrib['Waveform_1Hz'][key].iteritems():
	...
	for key in Wfm_20Hz_keys:
		...
		if key in ('Waveform','Coherence','Phase_diff'):
			...
			#-- add HDF5 variable attributes
			for att_name,att_val in CS_l1b_attrib['Waveform_20Hz'][key].iteritems():
			...
			for ds_name in beam_keys:
				...
				h5['Waveform_20Hz'][key][ds_name] = fileID.create_dataset(
					'Waveform_20Hz/%s/%s' % (key,ds_name), (n_records,n_blocks,),
				...
				#-- add HDF5 variable attributes
				for att_name,att_val in CS_l1b_attrib['Waveform_20Hz'][key][ds_name].iteritems():
	...
	if (HEADER == 1):
		...
		for att_name,att_val in CS_l1b_mds['METADATA']['MPH'].iteritems():
			fileID['METADATA']['MPH'].attrs[att_name] = att_val
		#-- Specific Product Header (SPH) are both strings and dictionaries
		for att_name,att_val in CS_l1b_mds['METADATA']['SPH'].iteritems():
		...
		for att_name,att_val in CS_l1b_mds['METADATA']['DSD'].iteritems():
		...
		for fi in CS_l1b_mds['METADATA']['MPH'].keys():
			...
			for att_name,att_val in CS_l1b_mds['METADATA']['MPH'][fi].iteritems():
				fileID['METADATA']['MPH'][fi].attrs[att_name] = att_val
		#-- Specific Product Header (SPH) are both strings and dictionaries
		for fi in CS_l1b_mds['METADATA']['SPH'].keys():
			fileID['METADATA']['SPH'].create_group(fi)
			for att_name,att_val in CS_l1b_mds['METADATA']['SPH'][fi].iteritems():
				if isinstance(att_val,dict):
					#-- if att_val is dictionary
		...
		for fi in CS_l1b_mds['METADATA']['DSD'].keys():
			...
			for att_name,att_val in CS_l1b_mds['METADATA']['DSD'][fi].iteritems():
...
def read_HDF5_cryosat_L1b(FILENAME, ATTRIBUTES='Y', VERBOSE='N'):
	...
	for key in fileID['Waveform_20Hz'].keys():
		if key in ('Waveform','Coherence','Phase_diff'):
			...
			for ds_name,ds_val in fileID['Waveform_20Hz'][key].iteritems():
	...
	if ATTRIBUTES in ('Y','y'):
		...
		for key in fileID['Waveform_1Hz'].keys():
			...
			for att_name,att_val in fileID['Waveform_1Hz'][key].attrs.iteritems():
		...
		for key in fileID['Waveform_20Hz'].keys():
			if key in ('Beam'):
				...
				for dsn in fileID['Waveform_20Hz'][key].keys():
					...
					for atn,atv in fileID['Waveform_20Hz'][key][dsn].attrs.iteritems():
						CS_l1b_mds['Attributes']['Waveform_20Hz'][key][dsn][atn] = atv
			else:
				CS_l1b_mds['Attributes']['Waveform_20Hz'][key] = {}
				for atn,atv in fileID['Waveform_20Hz'][key].attrs.iteritems():
