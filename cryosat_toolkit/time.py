...
def get_leap_seconds():
    ...
    """
    leaps=cryosat_toolkit.utilities.get_data_path(['data','leap-seconds.list'])
    #-- find line with file expiration as delta time
    with open(leaps,'r') as fid:
    ...
    #-- get leap seconds
    leap_UTC,TAI_UTC=np.loadtxt(cryosat_toolkit.utilities.get_data_path(leaps)).T
...
def update_leap_seconds(verbose=False, mode=0o775):
    ...
    try:
        cryosat_toolkit.utilities.from_ftp(HOST,timeout=20,
            local=cryosat_toolkit.utilities.get_data_path(LOCAL),hash=HASH,
    ...
    try:
        cryosat_toolkit.utilities.from_http(REMOTE,timeout=5,
            local=cryosat_toolkit.utilities.get_data_path(LOCAL),hash=HASH,
            verbose=verbose,mode=mode)
    except:
        pass
